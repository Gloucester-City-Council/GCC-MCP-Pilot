...
async function getAttachment(councilName, attachmentId) {
    ...
    const attachmentIdString = String(attachmentId).trim();
    if (!/^\d+$/.test(attachmentIdString)) {
        return {
            error: 'Invalid attachment_id',
    ...
    const parsedAttachmentId = parseInt(attachmentIdString, 10);
    if (parsedAttachmentId <= 0) {
        return {
            error: 'Invalid attachment_id',
            hint: 'attachment_id must be greater than 0'
        };
    }
    ...
    let result;
    try {
        result = await moderngovClient.getAttachment(councilName, parsedAttachmentId);
    } catch (e) {
        return {
            ...
            hint: 'The ModernGov API is temporarily unavailable. Please try again or use get_meeting_details to verify the attachment ID.',
    ...
    const attachmentNotFound =
        ...
        Number.isNaN(result.attachmentid) ||
        result.attachmentid !== parsedAttachmentId;
    ...
    if (attachmentNotFound) {
        return {
            ...
            council: councilName,
            attachment_id: parsedAttachmentId,
            hint: 'Use get_meeting_details to discover valid attachment IDs from linked documents.'
        };
    }
