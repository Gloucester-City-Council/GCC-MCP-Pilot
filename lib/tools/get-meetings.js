...
async function getMeetings(councilName, committeeId, fromDate, toDate) {
    ...
    // The ModernGov API treats the range as exclusive of the end date when both
    // bounds are the same (single-day queries miss that day otherwise).  For
    // range queries the API is inclusive of to_date, so no adjustment is needed.
