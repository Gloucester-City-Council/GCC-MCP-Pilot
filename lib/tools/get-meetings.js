...
>>>>>>> parent of 9d9149e (Fix Azure meeting date parsing and response range handling)
...
<<<<<<< HEAD
    // If from_date and to_date are identical, extend to_date by one calendar day so
    // ModernGov returns meetings on that day (the API treats the range as exclusive
    // of the end date when both bounds are the same).
    if (fromDate && toDate && fromDate === toDate) {
        const [dd, mm, yyyy] = toDate.split('/').map(Number);
        const d = new Date(yyyy, mm - 1, dd);
        d.setDate(d.getDate() + 1);
        const pad = n => String(n).padStart(2, '0');
        toDate = `${pad(d.getDate())}/${pad(d.getMonth() + 1)}/${d.getFullYear()}`;
=======
