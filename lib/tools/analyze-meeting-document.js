...
function deriveCouncilFromUrl(url) {
    ...
    // Parse the input URL hostname for exact matching (prevents subdomain/path bypass attacks)
    let inputHost;
    try {
    ...
    // Try to match URL against council base URLs using exact hostname comparison
    for (const councilName of councilNames) {
        const council = councilConfig.getCouncil(councilName);
        if (!council) continue;

        try {
            const councilHost = new URL(council.url).hostname.toLowerCase();
            if (inputHost === councilHost) {
                return { council: council.name, isKnownCouncil: true };
            }
        } catch (e) {
            // Skip councils with unparseable URLs
        }
    }
...
async function analyzeMeetingDocument(url, extractSections = ['all'], maxItems = MAX_ITEMS_DEFAULT) {
    const warnings = [];
    const recommendationsDelegated = extractSections.includes('all') || extractSections.includes('recommendations');
    ...
    // Derive council from URL - must match a known Gloucestershire council hostname
    const councilInfo = deriveCouncilFromUrl(url);
    if (!councilInfo.isKnownCouncil) {
        return {
            success: false,
            error: 'Document URL does not match a recognised Gloucestershire council',
            error_type: 'invalid_input',
            suggestion: 'Only documents from Gloucestershire council ModernGov systems are supported. Use get_attachment to retrieve a valid document URL.'
        };
    }
    ...
    const result = {
        ...
        metadata: {
            ...
            source_url: url,
            recommendations_tool: 'get_report_recommendations'
        },
        ...
        content_guidance: {
            note: 'Use analyze_meeting_document for paper metadata and non-recommendation sections. For recommendation extraction, use get_report_recommendations. When presenting content from official_sections, quote verbatim using blockquotes. Provide plain English explanations separately. Always include source_url link.',
            format_example: 'Official Record: "[exact quote]" | In Plain English: [your explanation] | Source: [link] | Recommendations Tool: get_report_recommendations'
        }
    };

    if (recommendationsDelegated) {
        warnings.push('Recommendation extraction is now handled by get_report_recommendations. Use this tool for official recommendation wording and confidence evidence.');
    }
    ...
    // Store recommendation extraction evidence separately for operational safety
    if (result.sections && result.sections.recommendation_extraction) {
        result.metadata.recommendation_extraction = result.sections.recommendation_extraction;
    }
    ...
    if (documentType === 'committee_report') {
        ...
        if (!result.sections.recommendations || result.sections.recommendations.length === 0) {
            warnings.push("No formal Cabinet/Council recommendation wording detected");
        }
    }
...
function extractReportSections(text, requestedSections, extractAll, maxItems) {
    ...
    const recommendationExtraction = extractCabinetRecommendationData(text, maxItems);
    sections.cabinet_recommendation_text = recommendationExtraction.cabinet_recommendation_text;
    sections.report_recommendations_or_conclusions = recommendationExtraction.report_recommendations_or_conclusions;
    sections.recommendations = recommendationExtraction.recommendations;
    sections.recommendation_extraction = recommendationExtraction.extraction;
    ...
    for (let i = 0; i < sectionPositions.length; i++) {
        ...
        // Special handling for recommendations - driven by formal recommendation extractor
    ...
    // Backward compatibility fallback for legacy clients
    if ((!sections.recommendations || sections.recommendations.length === 0) && sections.cabinet_recommendation_text) {
        sections.recommendations = parseRecommendations(sections.cabinet_recommendation_text, maxItems);
    }

    return sections;
}

function extractCabinetRecommendationData(text, maxItems) {
    const normalizedText = normalizePdfExtractedText(text);
    const lines = normalizedText.split('\n');
    const lineMeta = buildLineMetadata(normalizedText);
    const recommendationsHeadingPattern = /^\s*(?:\d+(?:\.\d+)*)?\s*recommendations\s*$/i;
    const rejectNarrativePattern = /future\s+work|conclusions?|next\s+steps/i;
    const decisionTriggerPatterns = [
        /\b(cabinet|council|committee)\s+is\s+asked\s+to\s+resolve\s+that\b/i,
        /\bthe\s+committee\s+is\s+recommended\s+to\b/i,
        /\b(cabinet|council|committee)\s+is\s+recommended\s+to\b/i,
        /\b(cabinet|council|committee)\s+is\s+asked\s+to\b/i,
        /\b(cabinet|council|committee)\s+is\s+(asked|recommended)\s+to\b/i,
        /\brecommended\s+that\b/i,
        /\bresolve\s+that\b/i,
        /\bresolved\s+that\b/i,
        /\bthat\s+(cabinet|council|committee)\b/i
    ];
    const itemPattern = /^\s*(?:\(\d+\)|\d+(?:\.\d+)*[.)]?|[a-z][.)]|[•\-])\s*/i;
    const triggerWindowLines = 12;
    const formalCandidatePattern = /^(?:that\s+(?:cabinet|council|committee)\b|(?:the\s+)?(?:cabinet|council|committee)\s+is\s+(?:asked|recommended)\s+to\b|resolved\s+that\b|recommended\s+that\b)/i;

    const narrativeBlocks = [];
    const formalRecommendations = [];
    const evidencePages = [];
    let headingDetected = null;
    let decisionTrigger = null;

    function isSectionBoundary(line) {
        if (!line) return false;
        if (recommendationsHeadingPattern.test(line)) return false;
        if (/^\d+(?:\.\d+)*\s+(?:(?:the\s+)?(?:cabinet|council|committee)\b|that\b)/i.test(line)) return false;

        if (/^\d+(?:\.\d+)*\s+(?:reason|background|financial|legal|risk|options|conclusions?|decision|resolutions?)\b/i.test(line)) {
            return true;
        }

        if (/^(reason for report|purpose of report|financial implications?|legal implications?|risk assessment|background|conclusions?|decision|resolutions?)\s*:?\s*$/i.test(line)) {
            return true;
        }

        return /^\d+(?:\.\d+)*\s+[A-Z][A-Za-z\s&\-/,()':]{3,120}:?$/.test(line);
    }

    function normalizeWhitespaceForMatching(input) {
        if (!input) return '';
        return input
            .replace(/[\u00A0\u1680\u180E\u2000-\u200B\u202F\u205F\u3000\uFEFF]/g, ' ')
            .replace(/[ \t\v\f\r]+/g, ' ')
    ...
    function detectDecisionTrigger(windowText) {
        const normalizedWindow = normalizeWhitespaceForMatching(windowText);
        for (const pattern of decisionTriggerPatterns) {
            const match = normalizedWindow.match(pattern);
            if (match) {
                return {
                    matched: true,
                    matchedText: match[0],
    ...
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i].trim();
        if (!recommendationsHeadingPattern.test(line)) continue;

        const page = lineMeta[i]?.page || 1;
        if (!headingDetected) {
            headingDetected = line;
        }

        let triggerIndex = -1;
        for (let j = i + 1; j < lines.length && j <= i + triggerWindowLines; j++) {
            const nextLine = lines[j]?.trim() || '';
            if (!nextLine) continue;
            if (isSectionBoundary(nextLine)) break;
            const slidingWindow = [];
            let bestTriggerMatch = null;
            for (let k = j; k < lines.length && k <= Math.min(j + 4, i + triggerWindowLines); k++) {
                const candidate = lines[k]?.trim() || '';
                if (!candidate) continue;
                if (isSectionBoundary(candidate)) break;
                slidingWindow.push(candidate);
                const triggerMatch = detectDecisionTrigger(slidingWindow.join('\n'));
                if (!triggerMatch.matched) continue;

                bestTriggerMatch = triggerMatch;
                if (/\b(resolve|resolved)\s+that\b/i.test(triggerMatch.matchedText || '')) {
                    break;
                }
            }
            ...
            if (bestTriggerMatch) {
                triggerIndex = j;
                if (!decisionTrigger) {
                    decisionTrigger = bestTriggerMatch.matchedText;
                }
                break;
            }
        }
        ...
        const buffer = [];
        for (let j = i + 1; j < lines.length && buffer.length < 60; j++) {
            const nextLine = lines[j].trim();
            if (!nextLine) {
                continue;
            }
        ...
        if (triggerIndex !== -1) {
            const triggerBufferOffset = Math.max(0, triggerIndex - (i + 1));
            const triggerBuffer = buffer.slice(triggerBufferOffset);
            const normalizedDecisionTrigger = normalizeWhitespaceForMatching(decisionTrigger || '');
            const listDecisionMode = /\b(resolve|resolved|recommended)\s+that\b/i.test(normalizedDecisionTrigger) || /\bis\s+(asked|recommended)\s+to\b/i.test(normalizedDecisionTrigger);

            const extractedItems = [];
            const formalLineSet = new Set();
            let currentItem = '';
            for (const rawLine of triggerBuffer) {
                const candidateLine = rawLine.trim();
                if (!candidateLine || rejectNarrativePattern.test(candidateLine)) continue;
                const normalizedCandidateLine = normalizeWhitespaceForMatching(candidateLine);

                if (itemPattern.test(candidateLine)) {
                    if (currentItem) extractedItems.push(cleanText(currentItem));
                    const strippedItem = candidateLine.replace(itemPattern, '').trim();
                    if (listDecisionMode || formalCandidatePattern.test(strippedItem)) {
                        currentItem = strippedItem;
                        formalLineSet.add(candidateLine);
                    } else {
                        currentItem = '';
                    }
                } else if (currentItem) {
                    currentItem += ` ${candidateLine}`;
                    formalLineSet.add(candidateLine);
                } else if (detectDecisionTrigger(normalizedCandidateLine).matched) {
                    continue;
                }
            }
            if (currentItem) extractedItems.push(cleanText(currentItem));
            ...
            for (const item of extractedItems) {
                if (!item) continue;
                formalRecommendations.push(item);
            }
            ...
            const narrativeLines = buffer.filter(l => !detectDecisionTrigger(l).matched && !formalLineSet.has(l));
            if (narrativeLines.length > 0) {
                narrativeBlocks.push({
                    text: cleanText(narrativeLines.join('\n')),
                    page,
                    heading: line
                });
            }
        ...
        const narrativeLines = buffer.filter(l => !detectDecisionTrigger(l).matched);
        if (narrativeLines.length > 0) {
            narrativeBlocks.push({
                text: cleanText(narrativeLines.join('\n')),
                page,
    ...
    const uniqueFormalRecommendations = [...new Set(formalRecommendations.filter(Boolean))]
        .slice(0, maxItems);
    const confidence = headingDetected && decisionTrigger && uniqueFormalRecommendations.length > 0
        ? 'high'
        : headingDetected && decisionTrigger
            ? 'medium'
            : 'low';
    const sortedEvidencePages = [...new Set(evidencePages)].sort((a, b) => a - b);

    const cabinetRecommendationText = uniqueFormalRecommendations.length > 0
        ? uniqueFormalRecommendations.join('\n')
        : null;

    const narrativeText = narrativeBlocks.length > 0
        ? [...new Set(narrativeBlocks.map(b => b.text).filter(Boolean))].join('\n\n')
        : null;

    return {
        recommendations: uniqueFormalRecommendations,
        cabinet_recommendation_text: cabinetRecommendationText,
        report_recommendations_or_conclusions: narrativeText,
        extraction: {
            confidence,
            evidence_pages: sortedEvidencePages,
            heading_detected: headingDetected,
            decision_trigger: decisionTrigger,
            message: cabinetRecommendationText
                ? 'Formal recommendation wording detected under Recommendations heading'
                : 'No formal Cabinet/Council recommendation wording detected under Recommendations heading'
        }
    };
}

function normalizePdfExtractedText(input) {
    if (!input) return '';
...
function buildLineMetadata(text) {
...
module.exports = {
    analyzeMeetingDocument,
    _internal: {
        extractCabinetRecommendationData,
