...
function extractCabinetRecommendationData(text, maxItems) {
    const lines = text.split('\n');
    const lineMeta = buildLineMetadata(text);
    ...
    const itemPattern = /^\s*(?:\(\d+\)|\d+(?:\.\d+)*[.)]?|[•\-])\s+/;
    const triggerWindowLines = 12;
    const formalCandidatePattern = /^(?:that\s+(?:cabinet|council|committee)\b|(?:cabinet|council|committee)\s+is\s+(?:asked|recommended)\s+to\b|resolved\s+that\b)/i;
    ...
    function isSectionBoundary(line) {
        ...
        return /^\d+(?:\.\d+)*\s+[A-Z][A-Za-z\s&\-/]{3,80}:?$/.test(line);
    }
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (triggerIndex !== -1) {
            ...
            const listDecisionMode = /\b(resolve|resolved)\s+that\b/i.test(normalizedDecisionTrigger) || /\bis\s+(asked|recommended)\s+to\b/i.test(normalizedDecisionTrigger);
