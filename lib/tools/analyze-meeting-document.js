...
async function analyzeMeetingDocument(url, extractSections = ['all'], maxItems = MAX_ITEMS_DEFAULT) {
    ...
    if (documentType === 'committee_report') {
        ...
        if (!result.sections.recommendations || result.sections.recommendations.length === 0) {
            warnings.push("No formal Cabinet recommendation wording detected");
        }
    }
...
function extractCabinetRecommendationData(text, maxItems) {
    ...
    const decisionVerbPattern = /\b(approv\w*|adopt\w*|not\w*|agre\w*|endors\w*|delegat\w*|resolv\w*|recommend\w*)\b/i;
    const formalStartPattern = /^(?:\d+(?:\.\d+)*[.)]?\s*)?(that\s+cabinet\b|cabinet\s+is\s+recommended\s+to\b|resolved\s+that\b)/i;
    const rejectNarrativePattern = /future\s+work|conclusions?|next\s+steps/i;
    const headingPattern = /^(?:\d+(?:\.\d+)*\s+)?(recommendations?|decision|resolutions?)\s*:?[\s.]*$/i;
    const narrativeHeadingPattern = /^(?:\d+(?:\.\d+)*\s+)?(?:future\s+work(?:\s+and)?\s+conclusions?|conclusions?|next\s+steps)\s*:?\s*$/i;

    const formalBlocks = [];
    const narrativeBlocks = [];
    let headingDetected = null;

    // Strategy 1: explicit decision wording anywhere in the document
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i].trim();
        if (!formalStartPattern.test(line)) continue;

        const buffer = [line];
        let j = i + 1;
        while (j < lines.length && buffer.length < 10) {
            const nextLine = lines[j].trim();
            if (!nextLine) break;
            if (/^(?:\d+(?:\.\d+)*\s+)?[A-Z][A-Za-z\s&\-/]{3,80}:?$/.test(nextLine) && !decisionVerbPattern.test(nextLine)) break;
            buffer.push(nextLine);
            j++;
        }

        const candidate = cleanText(buffer.join('\n'));
        if (candidate && decisionVerbPattern.test(candidate) && !rejectNarrativePattern.test(candidate)) {
            formalBlocks.push({
                text: candidate,
                page: lineMeta[i]?.page || 1,
                heading: 'decision_phrase'
            });
        }
    }

    // Strategy 2: recommendations/decision heading on early pages (1-2)
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i].trim();
        const page = lineMeta[i]?.page || 1;
        if (!headingPattern.test(line)) continue;

        if (page <= 2 && !headingDetected) {
            headingDetected = line;
        }

        const buffer = [];
        for (let j = i + 1; j < lines.length && buffer.length < 30; j++) {
            const nextLine = lines[j].trim();
            if (!nextLine) {
                if (buffer.length > 0) break;
                continue;
            }
            if (/^(?:\d+(?:\.\d+)*)\s+[A-Z][A-Za-z\s&\-/]{3,80}:?$/.test(nextLine) && !/^\d+(?:\.\d+)*[.)]?\s+(?:that\s+cabinet|cabinet\s+is\s+recommended\s+to)/i.test(nextLine)) {
                break;
            }
            buffer.push(nextLine);
        }
        ...
        if (page <= 2 && decisionVerbPattern.test(block) && !rejectNarrativePattern.test(block)) {
            formalBlocks.push({ text: block, page, heading: line });
        } else if (page > 2 || rejectNarrativePattern.test(block)) {
            narrativeBlocks.push({ text: block, page, heading: line });
        }
    }


    // Strategy 3: explicitly capture narrative recommendation/conclusion sections
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i].trim();
        const page = lineMeta[i]?.page || 1;
        if (!narrativeHeadingPattern.test(line)) continue;

        const buffer = [];
        for (let j = i + 1; j < lines.length && buffer.length < 25; j++) {
            const nextLine = lines[j].trim();
            if (!nextLine) {
                if (buffer.length > 0) break;
                continue;
            }
            if (/^(?:\d+(?:\.\d+)*)\s+[A-Z][A-Za-z\s&\-/]{3,80}:?$/.test(nextLine)) break;
            buffer.push(nextLine);
        }
    ...
    const uniqueFormal = [...new Set(formalBlocks.map(b => b.text))];
    const formalRecommendations = uniqueFormal.length > 0
        ? [...new Set(uniqueFormal.flatMap(block => parseRecommendations(block, maxItems)).filter(Boolean))]
        : [];

    const confidence = formalRecommendations.length > 0 ?
        (formalBlocks.some(b => b.page <= 2) ? 'high' : 'medium') : 'low';

    const evidencePages = [...new Set(formalBlocks.map(b => b.page))].sort((a, b) => a - b);
    const cabinetRecommendationText = formalRecommendations.length > 0
        ? formalRecommendations.join('\n')
        : null;

    const narrativeText = narrativeBlocks.length > 0
        ? [...new Set(narrativeBlocks.map(b => b.text))].join('\n\n')
        : null;

    return {
        recommendations: formalRecommendations.slice(0, maxItems),
        ...
        extraction: {
            ...
            message: cabinetRecommendationText
                ? 'Formal Cabinet recommendation wording detected'
                : 'No formal Cabinet recommendation wording detected'
        }
    };
}
