...
function extractCabinetRecommendationData(text, maxItems) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        for (let j = i + 1; j < lines.length && j <= i + triggerWindowLines; j++) {
            ...
            if (decisionTriggerPatterns.some(pattern => pattern.test(nextLine))) {
                triggerIndex = j;
                if (!decisionTrigger) {
                    decisionTrigger = nextLine;
                }
                break;
            }
        }
        ...
        if (triggerIndex !== -1) {
            ...
            const resolveMode = /\bresolve\s+that\b/i.test(decisionTrigger || '');
            ...
            for (const rawLine of triggerBuffer) {
                ...
                if (itemPattern.test(candidateLine)) {
                    ...
                    if (resolveMode || formalCandidatePattern.test(strippedItem)) {
                    ...
                    formalLineSet.add(candidateLine);
                } else if (decisionTriggerPatterns.some(pattern => pattern.test(candidateLine))) {
                    continue;
                }
            }
            if (currentItem) extractedItems.push(cleanText(currentItem));
            ...
            const narrativeLines = buffer.filter(l => !decisionTriggerPatterns.some(pattern => pattern.test(l)) && !formalLineSet.has(l));
        ...
        const narrativeLines = buffer.filter(l => !decisionTriggerPatterns.some(pattern => pattern.test(l)));
        if (narrativeLines.length > 0) {
            narrativeBlocks.push({
                text: cleanText(narrativeLines.join('\n')),
                page,
