...
function extractCabinetRecommendationData(text, maxItems) {
    ...
    const formalStartPattern = /^(?:\d+(?:\.\d+)*[.)]?\s*)?(that\s+(?:cabinet|council|committee)\b|(?:cabinet|council|committee)\s+is\s+recommended\s+to\b|resolved\s+that\b)/i;
    const recommendationsHeadingPattern = /^(?:\d+(?:\.\d+)*\s+)?recommendations?\s*:?\s*$/i;
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        const buffer = [];
        for (let j = i + 1; j < lines.length && buffer.length < 40; j++) {
        ...
        const numberedLines = buffer.filter(l => /^\d+(?:\.\d+)*[.)]?\s+/.test(l));
        const formalLines = numberedLines.filter(l => formalStartPattern.test(l) && !rejectNarrativePattern.test(l));

        if (formalLines.length > 0) {
            formalBlocks.push({
                text: cleanText(formalLines.join('\n')),
                page,
                heading: line
            });
        }

        const narrativeLines = buffer.filter(l => !formalLines.includes(l));
    ...
    const uniqueFormal = [...new Set(formalBlocks.map(b => b.text).filter(Boolean))];
    ...
    const confidence = formalRecommendations.length > 0 ? 'high' : 'low';
