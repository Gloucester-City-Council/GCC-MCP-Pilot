...
async function analyzeMeetingDocument(url, extractSections = ['all'], maxItems = MAX_ITEMS_DEFAULT) {
    ...
    if (documentType === 'committee_report') {
        ...
        confidenceAssessment = assessConfidence({ amendment_text: result.amendment?.amendment_text, amended_motion: result.amendment?.amended_motion }, 'amendment');
...
function extractAmendment(text) {
    ...
    // Find title (often after "Amendment:" or "Amendment to:")
    const titleMatch = text.match(/amendment\s*(?:to)?\s*[:–-]?\s*([^\n]+)/i);
    if (titleMatch) {
        amendment.title = titleMatch[1].trim();
    }

    // Try to extract the amendment text
    // Look for patterns like "That the motion be amended to read:" or "delete ... and insert ..."
    ...
    if (amendedToReadMatch) {
        amendment.amendment_text = cleanText(amendedToReadMatch[1]);
    }

    // Try to find delete/insert pattern
    ...
    if (deleteInsertMatch) {
        ...
        amendment.amendment_text = deleteInsertMatch[2].trim();
        amendment.amendment_type = 'substitute';
    }

    // Look for "That this Council" or similar amended motion text
    ...
    // Fallback: try to extract from "This Council" if present (common in amended motions)
    if (!amendment.amended_motion && !amendment.amendment_text) {
        const thisCouncilIndex = text.search(/this council/i);
        if (thisCouncilIndex !== -1) {
            let motionText = text.substring(thisCouncilIndex);
...
function generateAmendmentSummary(amendment) {
    ...
    if (amendment.amended_motion) {
        ...
        parts.push(`Amended text contains ${wordCount} words.`);
    }
