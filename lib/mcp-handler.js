...
const { getMeetingBriefing } = require('./tools/get-meeting-briefing');
...
This is the preferred tool for recommendation wording. It returns formal recommendation evidence and confidence metadata intended for verbatim quoting in responses.`,
    ...
    },
    {
        ...
        description: `Builds an accountability-focused, presentation-ready meeting briefing for every agenda item.

This tool combines official meeting record data with document extraction to produce structured outputs that distinguish:
- Official record (verbatim fields)
- Plain English summary
- Explicit inference

Use when you need complete member/public briefing packs with evidence gaps and source links per agenda item.

Usage example: get_meeting_briefing(council_name='Gloucester City Council', meeting_id=123456)`,
        inputSchema: {
            type: 'object',
            properties: {
                council_name: {
                    type: 'string',
                    description: `Council name. Must match exactly (case-sensitive). Available councils: ${councilConfig.getCouncilNames().join(', ')}`,
                    enum: councilConfig.getCouncilNames()
                },
                meeting_id: {
                    type: 'integer',
                ...
                include_document_analysis: {
                    type: 'boolean',
                    description: 'Whether to analyse linked PDF reports for deeper extraction (default: true).'
                }
            },
            required: ['council_name', 'meeting_id']
        }
...
async function callTool(name, args, context) {
    ...
    switch (name) {
        ...
        case 'get_meeting_briefing':
            return await getMeetingBriefing(
                args.council_name,
                args.meeting_id,
                args.include_document_analysis !== false
            );
