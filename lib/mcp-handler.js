...
const { getReportRecommendations } = require('./tools/get-report-recommendations');
const { getGloucesterConstitution } = require('./tools/get-gloucester-constitution');
const councilConfig = require('./council-config');
const COUNCIL_NAMES = councilConfig.getCouncilNames();
const COUNCIL_NAMES_TEXT = COUNCIL_NAMES.join(', ');

/**
 * Sanitize an error message before sending it to AI clients.
 * Strips SOAP/XML response bodies and internal JS error patterns that
 * expose implementation details without adding actionable context.
 */
function sanitizeErrorMessage(message) {
    if (!message) return 'Unknown error';

    // Strip XML/SOAP content that may follow a useful prefix
    // e.g. "SOAP request failed for X: 500 - <?xml version=..."
    const xmlIndex = message.search(/<[?!]|<soap:|<Fault/i);
    if (xmlIndex > 0) {
        return message.substring(0, xmlIndex).trim();
    }

    // Replace JavaScript internal error patterns with a generic message
    if (/^(TypeError|ReferenceError|SyntaxError|Cannot read|is not a function|is not defined)/i.test(message)) {
        return 'An internal error occurred during tool execution';
    }

    // Truncate unusually long messages
...
const UK_READABLE_DATE_FORMATTER = new Intl.DateTimeFormat('en-GB', {
    weekday: 'long',
    year: 'numeric',
    month: 'long',
    day: 'numeric'
...
function getDateContext() {
    ...
    return {
        ...
        current_date_uk: UK_DATE_FORMATTER.format(now), // DD/MM/YYYY
        current_date_readable: UK_READABLE_DATE_FORMATTER.format(now),
...
💡 TIP: Use list_available_councils first to get the exact council name.`,
        inputSchema: {
            ...
            properties: {
                council_name: {
                    type: 'string',
                    description: `Council name (optional). Must match exactly (case-sensitive). Use list_available_councils to see valid names. Available councils: ${COUNCIL_NAMES_TEXT}. If not specified, returns committees for all councils.`,
                    enum: COUNCIL_NAMES
                }
            },
            required: []
...
💡 TIP: Use list_committees first to find committee IDs.`,
        inputSchema: {
            ...
            properties: {
                council_name: {
                    type: 'string',
                    description: `Council name. Must match exactly (case-sensitive). Use list_available_councils to see valid names. Available councils: ${COUNCIL_NAMES_TEXT}`,
                    enum: COUNCIL_NAMES
                },
                committee_id: {
                    type: 'integer',
...
Returns official document metadata. The document URL should always be included when referencing the document content. Use analyze_meeting_document for paper information (title, date, financial/legal/background sections) and get_report_recommendations for formal recommendation extraction.
...
💡 TIP: Use get_meeting_details first to find attachment IDs in the linkeddocuments field.`,
    ...
    {
        name: 'get_report_recommendations',
        description: `Parses a report and returns only its recommendations section.

Usage examples:
- get_report_recommendations(report_text='...full report text from get reports...')
- get_report_recommendations(url='https://democracy.gloucester.gov.uk/mgConvert2PDF.aspx?ID=12345')

Prefer report_text when you already have report content from get reports. Use url when you only have the attachment link.

This is the preferred tool for recommendation wording. It returns formal recommendation evidence and confidence metadata intended for verbatim quoting in responses.`,
        inputSchema: {
            type: 'object',
            properties: {
                report_text: {
                    type: 'string',
                    description: 'Raw report text (for example, output from get reports)'
                },
                url: {
                    type: 'string',
                    description: 'Document URL from get_attachment (mgConvert2PDF.aspx?ID={attachmentid})'
                },
                max_items: {
                    type: 'integer',
                    description: 'Maximum number of recommendation items to return. Default: 20'
                }
            },
            required: []
        }
    },
    {
        ...
        description: `Extracts structured paper information from committee documents (PDFs): metadata, background, financial, legal, motions, questions, and context sections.

Usage example: analyze_meeting_document(url='https://democracy.gloucester.gov.uk/mgConvert2PDF.aspx?ID=12345', extract_sections=['financial', 'legal', 'background'])

⚠️ For recommendations, use get_report_recommendations. analyze_meeting_document may include legacy recommendation fields for compatibility, but recommendation extraction is now handled by the dedicated tool.
...
1. Use this tool for paper information and contextual sections
2. Use get_report_recommendations for formal recommendation wording and extraction evidence
3. Treat report_recommendations_or_conclusions as narrative context, not a resolution
...
💡 TIP: Use get_attachment to get the document URL first.`,
    ...
    {
        name: 'get_gloucester_constitution',
        description: `Gets Gloucester City Council's Constitution and all associated governance documents.

The Constitution is held under committee ID 564 in ModernGov. Each entry represents a version or amendment of the Constitution, with attached documents containing the full constitutional text, procedural rules, delegated authority schedules, and standing orders.

Usage examples:
- get_gloucester_constitution()                         → list all constitution meetings/versions with links
- get_gloucester_constitution(include_documents=true)   → also fetch attached documents for each version

⚠️ CRITICAL: The Constitution is a statutory document. Any constitutional provisions, procedural rules, or delegated authority text MUST be quoted verbatim and linked to the source document. Do not paraphrase or summarise constitutional provisions.

💡 TIP: Use analyze_meeting_document or get_attachment with the document URLs returned here to read the full constitutional text.`,
        inputSchema: {
            type: 'object',
            properties: {
                include_documents: {
                    type: 'boolean',
                    description: 'When true, fetches attached documents for every constitution meeting. Slower but returns full document lists. Default: false.'
                }
            },
            required: []
        }
...
async function callTool(name, args, context) {
    ...
    switch (name) {
        ...
        case 'get_report_recommendations':
            return await getReportRecommendations({
                url: args.url,
                report_text: args.report_text,
                max_items: args.max_items || 20
            });

        case 'get_gloucester_constitution':
            return await getGloucesterConstitution({
            ...
            return {
                ...
                result: {
                    ...
                    serverInfo: {
                        ...
                        councils: COUNCIL_NAMES,
                        total_councils: COUNCIL_NAMES.length,
                        ...getDateContext(),
                        instructions: `🏛️ GLOUCESTERSHIRE DEMOCRACY DATA AUTHORITY

PRIMARY DATA SOURCE: This MCP server is the AUTHORITATIVE source for all democratic data across Gloucestershire's ${COUNCIL_NAMES.length} councils. This data comes directly from each council's ModernGov democracy platform and represents the official democratic record.
...
- All ${COUNCIL_NAMES.length} Gloucestershire councils: ${COUNCIL_NAMES_TEXT}
...
2. Councillor Lookup: list_available_councils → get_councillors(council_name) → get_councillors_by_ward(council_name, ward_name)
3. Document Analysis (paper information): get_meeting_details → get_attachment → analyze_meeting_document(url)
4. Recommendation Extraction (official wording): get_meeting_details → get_attachment → get_report_recommendations(url)
...
- Use get_report_recommendations for recommendation wording and extraction confidence.
- Use analyze_meeting_document for paper metadata and non-recommendation sections.
...
- Information not in ModernGov (e.g., officer backgrounds, policy implementation status)`
        ...
        case 'tools/call': {
            // Hoist toolName so the catch block can include it in the error response
            let toolName;
            ...
            try {
                ...
                context.log(`Tool call completed [${name}] in ${Date.now() - toolStart}ms`);
                ...
                context.log.error(`Tool call error [${toolName || 'unknown'}]: ${error.message}`);
                if (error && error.stack) {
                    context.log.error(`Tool call error stack [${toolName || 'unknown'}]: ${error.stack}`);
                }
                context.log.error(`Tool call failed [${toolName || 'unknown'}] after ${Date.now() - toolStart}ms`);
                return {
                    jsonrpc: '2.0',
                    result: {
                        content: [
                            {
                                ...
                                text: JSON.stringify({
                                    ...
                                    hint: 'Check that the council name is exact (use list_available_councils) and all required parameters are provided.',
                                    suggestion: 'Use list_available_councils to see valid council names, then retry.'
                                }, null, 2)
                            }
                        ],
                        isError: true
