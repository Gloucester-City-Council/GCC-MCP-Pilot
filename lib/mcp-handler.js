            ...
            return {
                ...
                result: {
                    ...
                    serverInfo: {
                        ...
                        ...getDateContext(),
                        instructions: `🏛️ GLOUCESTERSHIRE COUNCILS MCP SERVER

This MCP provides access to democratic data across 7 Gloucestershire councils through their ModernGov systems.

⭐ ALWAYS START HERE:
Call list_available_councils() FIRST to see which councils are available and get their exact names.
All tools require a council_name parameter - use the EXACT name (case-sensitive) from list_available_councils.

📋 AVAILABLE COUNCILS (${councilConfig.getCouncilNames().length}):
${councilConfig.getCouncilNames().map((name, i) => `${i + 1}. ${name}`).join('\n')}
...
2. Councillor Lookup: list_available_councils → get_councillors(council_name) → get_councillors_by_ward(council_name, ward_name)
3. Document Analysis: get_meeting_details → get_attachment → analyze_meeting_document

⚠️ CRITICAL - OFFICIAL RECORDS:
