...
class ModernGovClient {
    ...
    async getMeeting(councilName, meetingId) {
        ...
        // ModernGov returns a ghost record (meetingid=0) for unknown IDs rather
        // than an error - detect this and surface a clear not-found signal.
        const resolvedId = parseInt(meeting?.meetingid, 10);
        if (!meeting || !resolvedId || resolvedId === 0) {
