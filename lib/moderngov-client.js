    ...
    async getCouncillorsByWard(councilName) {
        ...
        return {
            wards: wards.map(ward => ({
                ward_id: ward.wardid != null ? parseInt(ward.wardid, 10) : null,
    ...
    async getMeetings(councilName, committeeId, fromDate, toDate) {
        ...
        return {
            meetings: meetings.map(m => ({
                ...
                location: m.meetinglocation || null,
                status: m.meetingstatus || null,
                is_webcast: m.iswebcast === 'True',
                agenda_published: m.agendapublished === 'True'
            }))
        };
    }
...
<soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/"
  ...
  <soap:Body>
    ...
    /**
     * Make a SOAP request to ModernGov with exponential backoff retry
     *
     * @param {string} operation - SOAP operation name
     * @param {string} bodyContent - Inner body XML content
     * @param {string} councilName - Council name
    ...
    async _callSoap(operation, bodyContent = '', councilName) {
        ...
        for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
            try {
                ...
                // Strip DOCTYPE declarations before parsing to prevent XML entity expansion attacks.
                // The pattern handles both external subsets and internal subset [...] blocks.
                const safeXml = response.data.replace(/<!DOCTYPE[^[>]*(?:\[[^\]]*\])?\s*>/gi, '');
                const parsed = await this.parser.parseStringPromise(safeXml);
                return parsed;
            } catch (error) {
        ...
        if (lastError.response) {
            throw new Error(`SOAP request failed for ${councilName}: ${lastError.response.status} - ${lastError.response.data}`);
        }
        throw new Error(`SOAP request failed for ${councilName}: ${lastError.message}`);
    }
