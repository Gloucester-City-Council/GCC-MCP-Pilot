...
describe('gcc-procurement determine-route notice derivation', () => {
    ...
    test('treats waiver as a direct-award style route for notice derivation', () => {
        const directCodes = requiredNoticeCodes('direct_award');
        const waiverCodes = requiredNoticeCodes('waiver');

        expect(waiverCodes).toEqual(directCodes);
    });
