...
describe('gcc-procurement get-notices input validation and route handling', () => {
    ...
    test('accepts additional schema-listed procurement routes', () => {
        const res = execute({
            value_gbp: 50000,
            contract_type: 'services',
            procurement_route: 'budget_holder_discretion',
            response_format: 'json',
        });

        expect(res.ok).toBe(true);
        expect(res.result.procurement_route).toBe('budget_holder_discretion');
    });
